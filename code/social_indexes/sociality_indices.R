...
## The code below contains the fuctions that are used to calculate the DSI (and SCI) values.
## The code is based on the social index code of the Ramboseli packages.
## It was addapted to it could work for juvenile and calculate new dyadic types sucj as DSI paternal.
...
## SCI functions
get_sci_subset <- function(df, biograph_l, members_l, focals_l, females_l,
                           interactions_l, min_res_days, directional) {
    zero_daily_count <- zero_daily_count
	log_zero_daily_count <- log2(zero_daily_count)
	my_sname <- df$sname
	...
	# To allow animals to be non-adults only if focal is not adult
	...
	my_interactions_dad_excluded <- my_interactions %>%
		...
		mutate(paternal_grooms =
					 	case_when(
					 	    actor_sex_class == "JUV" ~ actee == actor_dad,
	...
	include_males <- my_interactions$act[[1]] != "G" |
	    (my_interactions$act[[1]] == "G" & df$sex_class != "AM")
	...
	if(df$sex_class == "JUV") {
		...
		gg_fme <- get_interaction_dates(my_subset, members_l,
		                                my_interactions_mom_excluded,
		                                quo(actee_sex), "actor", "F", TRUE) %>%
			dplyr::group_by(grp, sname) %>%
			dplyr::summarise(ItoFme = n(),  .groups = "drop")

		## Interactions received from females by that are not their mom
		gr_fme <- get_interaction_dates(my_subset, members_l,
		                                my_interactions_mom_excluded,
		                                quo(actor_sex), "actee", "F", TRUE) %>%
			dplyr::group_by(grp, sname) %>%
		...
		## Interactions given to juveniles by females that are not their mom
		gg_mde <- get_interaction_dates(my_subset, members_l,
		                                my_interactions_dad_excluded,
		                                quo(actee_sex), "actor", "M", TRUE) %>%
			dplyr::group_by(grp, sname) %>%
			dplyr::summarise(ItoMde = n(),  .groups = "drop")

		## Interactions received from females by that are not their mom
		gr_mde <- get_interaction_dates(my_subset, members_l,
		                                my_interactions_dad_excluded,
		                                quo(actor_sex), "actee", "M", TRUE) %>%
			dplyr::group_by(grp, sname) %>%
	...
	if (df$sex_class == "JUV") {
		...
		my_subset$SCI_Mde_Rec <- as.numeric(residuals(lm(data = my_subset, log2IfromMde_daily ~ log2OE)))
...
sci <- function(my_iyol, biograph_l, members_l, focals_l, females_l, interactions_l,
	...
	if (parallel) {
		...
		for (i in 1:nrow(my_iyol)) {
			my_iyol[i, ]$subset <- list(get_sci_subset(my_iyol[i, ], biograph_l,
			                                           members_l, focals_l, females_l,
			                                           interactions_l,
			                                           min_res_days = 60, directional = FALSE))
...
get_dyadic_subset <- function(df, biograph_l = biograph_l, members_l=members_l,
															focals_l =focals_l, females_l=females_l,
	...
	get_resident_overlap_dates <- function(list1, list2) {
		# Convert strings to Date objects
		dates1 <- as.Date(list1)
		dates2 <- as.Date(list2)

		# Get the common dates
		common_dates <- intersect(dates1, dates2)
		as.Date(common_dates, origin = "1970-01-01")
		}
	...
	write_message <-function(dyadic_message) {
		write.table(x = tibble(my_row = df$row_nr, my_message = dyadic_message),
								file = paste0('./data/dyadic_index_messages_', current_date, '.txt')
								, append = TRUE
								, row.names =FALSE,
								col.names = FALSE)
	}
	...
	print(paste("This is ", my_row, "XXX"))
	...
	my_subset <- my_subset %>%
		...
		inner_join(my_subset_list %>%
							 ...
							 , by = "temp_partner") %>%
		select(-tmp, !(contains("temp"))) ## getting rid off temp column
	...
	my_subset <- my_subset %>%
		mutate(resident_overlap = map2(.x = sname_resident_dates,
		                               ...
		                               .f = get_resident_overlap_dates)) %>%
		dplyr::mutate(days_present = map_int(.x = partner_resident_dates,
		                                     .f = length)) %>%
   	dplyr::mutate(coresidence_days = map_int(.x = resident_overlap,
   	                                         .f = length)) %>%
		dplyr::ungroup() %>%
	...
	if(my_subset %>%
		 filter((sname == df$sname & sname_grp == df$grp) |
		        (partner == df$sname & partner_grp == df$grp)) %>%
		 nrow() == 0) {
	...
	my_subset <- my_subset %>%
		dplyr::mutate(n_focals = purrr::pmap_dbl(.l = list(resident_overlap, sname_grp),
		                                         .f = get_focal_counts)) %>%
	...
	my_subset <- my_subset %>%
		dplyr::mutate(n_females = purrr::pmap_dbl(.l = list(resident_overlap, sname_grp),
		                                          .f = get_female_counts))
	...
	if (directional) {
		...
		if (nrow(my_subset) == 0) {
			...
			my_subset <- my_subset %>%
				mutate(i_given = pmap_dbl(
				    .l = list(sname_actor_interactions,	partner, partner_grp, partner_sex_class),
				    .f = get_i_given),
				    i_received = pmap_dbl(
				        .l = list(sname_actee_interactions, partner, partner_grp, partner_sex_class),
				        .f = get_i_received),
				    i_total = i_given + i_received) %>%
				mutate(i_total = if_else(i_total == 0, zero_daily_count, i_total)) %>%
			...
			add_all_options <- my_subset %>%
				ungroup() %>%
				slice(1:3) %>%
				mutate_all(, .funs = make_NA) %>%
				mutate(partner_sex_class = "JUV") %>%
				mutate(sname_sex_class = c("AF", "AM", "JUV")) %>%
				mutate(dyad_type = c("AF-JUV", "AM-JUV", "JUV-JUV")) %>%
				mutate(i_total = zero_daily_count) %>%
				mutate(i_adj = i_total / 365,
							 log2_i_adj = log2(i_adj))

			mean_AF_log2OE <- my_subset %>%
				filter(dyad_type == "AF-JUV") %>%
				group_by(partner, partner_grp) %>%
				summarise(mean_AF_log2OE = log2(mean(OE)), .groups = "drop")

			my_subset_data <- my_subset %>%
				...
				complete(partner, nesting(dyad_type),
								 fill = list(sname = 'XXX'
								             , i_total = zero_daily_count
								 			 , i_adj = zero_daily_count/365
								 			 , log2_i_adj = log2(zero_daily_count/365)
								 			 , n_focals = .1))

			my_subset <- my_subset %>%
				bind_rows(my_subset_data %>%
										anti_join(my_subset) %>%
										filter(sname != 'XXX' | partner != 'XXX') %>%
										dplyr::mutate(tmp = paste(sort(c(sname, partner)),
										                          collapse = '')))
			...
			my_subset <- my_subset %>%
				...
				mutate(log2OE = if_else(is.na(log2OE), mean_AF_log2OE, log2OE))
			...
			my_subset <- my_subset %>%
				dplyr::mutate(data = purrr::map(.x = data, .f = fit_dyadic_regression)) %>%
				tidyr::unnest(cols = c(data)) %>%
				ungroup()
		}
	}

	my_subset_reduced <- my_subset %>% select(tmp, sname, sname_sex, sname_sex_class,
	                                          partner, partner_sex, partner_sex_class,
	                                          sname_grp, partner_grp, dyad, dyad_type,
	                                          coresidence_days,	OE, log2OE, mean_AF_log2OE,
	...
	my_subset_paternal <- my_subset_reduced %>%
		...
		mutate(paternal_groom = case_when(
			...
			partner_sex_class == "JUV"  & sname_sex_class == "JUV" ~ "juvenile",
			TRUE ~ "other")) %>%
		filter(paternal_groom != 'other') %>%
		select(tmp, sname, partner, paternal_groom, sname_grp, partner_grp,
					 sname_sex_class, partner_sex_class, coresidence_days,
					 dyad_type, OE, log2OE, log2_i_adj, i_adj)

	if(!("AF-JUV" %in% unique(my_subset_paternal$dyad_type))) {
		message_text = c("No AF-JUV dyads present")
		write_message(message_text)
		#return(tibble::as_tibble(NULL))

	} else {

		add_all_options_paternal <- my_subset_paternal %>%
			ungroup() %>%
			slice(1:4) %>%
			mutate_all(, .funs = make_NA) %>%
			mutate(dyad_type = rep(c("AF-JUV", "AM-JUV"), each = 2)) %>%
			mutate(partner_sex_class = "JUV") %>%
			mutate(sname_sex_class = rep(c("AF", "AM"), each = 2)) %>%
			mutate(paternal_groom = c("maternal",
																"non_maternal",
																"paternal",
																"non_paternal")) %>%
			mutate(i_total = zero_daily_count) %>%
			mutate(coresidence_days = 0,
				i_adj = i_total / 365.25,
						 log2_i_adj = log2(i_adj)) %>%
			select(dyad_type, paternal_groom, everything())
		...
		paternal_data <- my_subset_paternal %>%
			bind_rows(add_all_options_paternal) %>%
			ungroup() %>%
			complete(paternal_groom,
							 ...
							 fill=list(partner = 'XXX',
							 					sname = 'XXX')) %>%
			group_by(partner_grp, sname_grp) %>%
			complete(partner, nesting(dyad_type, paternal_groom),
							 fill = list(sname = 'XXX'
							 						, coresidence_days = 0
							 						, i_total = zero_daily_count
							 						, i_adj = zero_daily_count/365
			...
			filter(!is.na(dyad_type) & partner != 'XXX') %>%
			filter(paternal_groom != 'other') %>%
			left_join(mean_AF_log2OE, by = c("partner", "partner_grp")) %>%
		...
		my_subset_paternal <- my_subset_paternal %>%
			bind_rows(paternal_data %>%
									anti_join(my_subset_paternal) %>%
									filter(sname != 'XXX' | partner != 'XXX') %>%
									dplyr::mutate(tmp = paste(sort(c(sname, partner)),
																						collapse = '')))


		paternal_regression <-my_subset_paternal %>%
			...
			select(contains("partner"),contains("sname")
						 , log2_i_adj,log2OE
						 , dyad_type,  paternal_groom,paternal_res_i_adj)

		my_subset_reduced <- my_subset_reduced %>%
		 full_join(paternal_regression)

		rm("my_subset_paternal", "paternal_data", "paternal_regression")
	...
	my_subset_reduced <- my_subset_reduced %>%
		group_by(dyad_type) %>%
		mutate_at(vars(contains('res_i_adj')), .funs = list(zscored = ~zscore(.)))
	...
	res <- tibble(nr_dyads = nrow(my_subset_reduced),
								nr_F_dyads = nrow(my_subset_reduced %>%
								                      filter(dyad_type == "AF-JUV")),
								my_subset_reduced %>%
									ungroup() %>%
									filter(focal_check) %>%
									nest(focal_data = everything())) %>%
		bind_cols(my_subset_reduced %>%
								filter(focal_check) %>%
								group_by(dyad_type) %>%
								arrange(-res_i_adj_zscored) %>%
								slice(1:3) %>%
								summarise(DSI = mean(res_i_adj_zscored)) %>%
								mutate(DSI_type =
											 	case_when(dyad_type == "AM-JUV" ~ "DSI_M",
											 						dyad_type == "AF-JUV" ~ "DSI_F",
											 						dyad_type == "JUV-JUV" ~ "DSI_J")) %>%
								select(-dyad_type) %>%
								pivot_wider(names_from = DSI_type, values_from = "DSI")) %>%
		bind_cols(my_subset_reduced %>%
								filter(focal_check) %>%
								group_by(paternal_groom) %>%
								arrange(-paternal_res_i_adj) %>%
								slice(1:3) %>%
								summarise(DSI = mean(paternal_res_i_adj)) %>%
								mutate(DSI_type = case_when(
								    paternal_groom == "juvenile" ~ "DSI_Jmat",
								...
								select(-paternal_groom) %>%
								pivot_wider(names_from = DSI_type, values_from = "DSI"))

	res <-res %>%
		rename_if(startsWith(names(.), "DSI"), ~paste0(., ".zscored"))

	temp <-bind_cols(df, res) %>%
		rename(focal = sname) %>%
		unnest(cols = focal_data) %>%
		select_if(negate(is_list))

	write.table(temp, file = paste0('./data/my_dsi_data_',
	                                latest_version_date, '.txt')
							, append = TRUE
							, row.names =FALSE
...
dyadic_index <- function(my_iyol, biograph_l, members_l, focals_l, females_l,
	...
	# Return an empty tibble if the subset is empty
	if (is.null(my_iyol) |
			!all(names(my_iyol) %in% c("sname", "grp", "start", "end",
			                           "days_present", "sex", "birth",
			                           "first_start_date", "statdate", "birth_dates",
	...
	if (parallel) {
		...
		cl <- makeCluster(ncores, outfile = "run_DSI_code.txt")
		registerDoSNOW(cl)
		pb <- txtProgressBar(min = 0, max = nrow(my_iyol), style = 3)
		progress <- function(n) setTxtProgressBar(pb, n)
		clusterExport(cl, list("get_dyadic_subset",
													 ...
													 "parents_l"));
		opts <- list(progress = progress)
		zscored_values <- foreach(i = 1:nrow(my_iyol),
		                          .options.snow = opts,
		                          .packages = c('tidyverse')) %dopar% {
		...
		close(pb)
		stopCluster(cl)
		my_iyol <- add_column(my_iyol, zscored_values)
		} else {
		if (!is.null(ncores)) {
			message("Ignoring 'ncores' argument because 'parallel' set to FALSE.")
		}
		my_iyol$zscored_values <- list(NULL)
		pb <- txtProgressBar(min = 0, max = nrow(my_iyol), style = 3) # Progress bar
		for (i in 1:nrow(my_iyol)) {
		    my_iyol[i, ]$zscored_DSI_values <- list(
		        get_dyadic_subset(my_iyol[i, ], biograph_l, members_l, focals_l, females_l,
		                          interactions_l, min_cores_days, within_grp, directional=FALSE))
		    setTxtProgressBar(pb, i)
		}
		close(pb)
		}
...
#my_subset_files,
dyadic_index.universal <- function(my_iyol,  biograph_l, members_l,
                                   focals_l, females_l, interactions_l,
								   min_cores_days = 60,
								   within_grp = FALSE, directional = FALSE,
 ...
 print("step1")
		...
		cl <- makeCluster(ncores, outfile = "dsi_universal_run")
   print("step2")
		registerDoSNOW(cl)
		pb <- txtProgressBar(min = 0, max = nrow(iyol_sub), style = 3)
    print("step3")
		progress <- function(n) setTxtProgressBar(pb, n)
		clusterExport(cl, list("universal_values"
													 , "iyol_sub"
													 , "parents_l"
													 , "adult_members"
													 ,	"zscore"
													 , "zero_daily_count"
													 , "add_variables"
													 , "get_universal_zscored"
													 , "get_paternal_position"
													 , "create_paternal_universal_zscored"
													 , "get_functions"
		));
		opts <- list(progress = progress)
   print("step4")
		universal_zscored_values <- foreach(i = 1:nrow(my_iyol),
		  .options.snow = opts,
		  .packages = c('tidyverse')) %dopar% {
			add_variables(my_iyol[i, ])
			}

            print("step4b")
			close(pb)
			stopCluster(cl)
			my_iyol <- add_column(my_iyol, universal_zscored_values)
			} else {
				if (!is.null(ncores)) {
					message("Ignoring 'ncores' argument because 'parallel' set to FALSE.")
					}
					my_iyol$universal_zscored_values <- list(NULL)
					pb <- txtProgressBar(min = 0, max = nrow(my_iyol), style = 3) # Progress bar
					for (i in 1:nrow(my_iyol)) {
						my_iyol[i, ]$universal_DSI_values <- list(get_dyadic_subset(
							my_iyol[i, ],
							biograph_l,
							members_l,
							focals_l,
...
get_universal_zscored <- function(df) {
	...
	df <- df %>%
		mutate(log2_i_adj = if_else(is.na(log2_i_adj), log(zero_daily_count), log2_i_adj)) %>%
		group_by(dyad_type) %>%
		mutate(universal_res_value =
					 	log2_i_adj - (B0 + (B1 * log2OE))) %>%
		ungroup() %>%
		filter(universal_res_value > -9999) %>%
		group_by(dyad_type) %>%
		mutate_at(vars(starts_with('universal_res_value')),
							.funs = list(universal_zscore = ~zscore(.))) %>%
		select(focal_check, sname, partner, dyad_type, paternal_groom,
					 log2OE, i_adj, log2_i_adj,
					 universal_res_value, universal_zscore)  %>%
		ungroup()
...
create_paternal_universal_zscored <- function(df) {
	...
	df_overall <- df %>%
		...
		group_by(dyad_type) %>%
		arrange(-universal_zscore) %>%
		slice(1:3) %>%
		summarise(DSI = mean(universal_zscore)) %>%
	...
	df_overall %>%
		...
		select_all(.funs = ~paste0(., ".universal"))
}
...
get_paternal_position <- function(df) {
	...
	df %>%
		filter(focal_check) %>%
		filter(universal_zscore > -999) %>%
		group_by(dyad_type) %>%
		arrange(dyad_type, -universal_zscore) %>%
		#select(focal_check, partner, sname, dyad_type, paternal_groom,zscore) %>%
		#mutate(position = percent_rank(zscore)) %>%
		mutate(position = cume_dist(universal_zscore)) %>%
...
get_functions <- function() source('./code/social_indexes/sociality_indices.R')

add_variables <- function(df) {

	file = df$file

	if(!(file %in% list.files('./data/my_subsets/'))) {
		res = tibble()
	} else {
		get_functions()
		res =read.table(paste0('./data/my_subsets/', file), header = TRUE) %>%
			as_tibble() %>%
			...
			nest() %>%
			mutate(universal_zscored_resvalues =
						 	map(.x = data,
						 			.f = get_universal_zscored))  %>%
			mutate(paternal_position = map(.x = universal_zscored_resvalues,
																		 .f = get_paternal_position)) %>%
			mutate(universal_zscored_DSI =
						 	map(.x = universal_zscored_resvalues,
						 			.f = create_paternal_universal_zscored)) %>%
			unnest(cols = c(universal_zscored_DSI)) %>%
			mutate(AF_partners = pmap(.l = list("F", focal, grp, universal_zscored_resvalues),
																.f = get_partners)) %>%
			mutate(AM_partners = pmap(.l = list("M", focal, grp, universal_zscored_resvalues),
																.f = get_partners)) %>%
			#select(-data) %>%
			...
			select(-potential_males, -potential_females, -data) %>%
...
get_top_grooms <- function(df, res_variable) {
	df %>%
		filter(focal_check) %>%
		filter(str_detect(paternal_groom, "paternal")) %>%
		group_by(paternal_groom) %>%
		arrange({{res_variable}}) %>%
		slice(1) %>%
		select(paternal_groom, grooming_partner = sname, {{res_variable}})  %>%
		ungroup()
}



##basedon 'old' DSI code
